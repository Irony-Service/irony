...
from irony.models.fetch_orders_vo import FetchOrderRequest, FetchOrderResponseBody, FetchOrdersResponse, OrderChunk
        ...
        pipeline =pipeline = [
    {
        "$match": {
            "order_status.0.status": {
                "$in": [
                     OrderStatusEnum.PICKUP_PENDING.value, OrderStatusEnum.WORK_IN_PROGRESS.value , OrderStatusEnum.DELIVERY_PENDING.value 
                ]
            },
            "service_location_id": {
                "$in":  agent.service_location_ids
...
def setResponseBody(orders):
    responseBody = FetchOrderResponseBody(pending_pick_up=[], work_in_progress=[], delivery_pending=[])
    for order in orders:
        order = Order(**order)
        if  order.order_status.__getitem__(0).status == OrderStatusEnum.PICKUP_PENDING.value:
            orderChunk = setModel(order)
            responseBody.pending_pick_up.append(orderChunk)
        elif order.order_status.__getitem__(0).status == OrderStatusEnum.WORK_IN_PROGRESS.value:
            orderChunk = setModel(order)
...
def setModel(order):
    ...
    if order is not None:
        orderChunk.order_id = str(order.id)
