...
from irony.models.service_agent.vo.register_agent_response import RegisterAgentData, RegisterAgentResponse
...
async def register_service_agent(user: ServiceAgentRegister) -> RegisterAgentResponse:
    try:
        ...
        return RegisterAgentResponse(message="Service Agent crerated successfully.", data=RegisterAgentData(user = service_agent))
...
async def login_service_agent(response: Response, user: UserLogin) -> LoginUserResponse:
    try:
        ...
        return LoginUserResponse(message="Success!", data = LoginUserData(access_token=token, token_type="bearer", user=db_user))
    except HTTPException:
        raise
    except Exception as e:
